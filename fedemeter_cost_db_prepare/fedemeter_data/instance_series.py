    ...
    for i in range(len(cloud_nd_array)):
        if provider == "aws":
            ...
            if "standard" in instance:
                ...
                family = instance_string_list[0] + "-" + "megamem"
            else:
                family = instance
            #print(instance,cpu,memory,family)
            provider_list.append(provider)
...
def to_csv(provider_list, instance_list, cpu_list, memory_list, series_list):

    instnace_family_dict = {"provider": provider_list, "instancetype": instance_list, "cpu": cpu_list, "memory": memory_list, "series": series_list}
    print(instnace_family_dict)

    instance_family_data = pd.DataFrame(instnace_family_dict, columns=["provider", "instancetype", "cpu", "memory", "series"])
    print(instance_family_data)

    instance_family_data.to_csv("C:\\Users\\Brian\\Desktop\\fedemeter_data\\instance_series.csv", index=False)

    instance_family_data.to_csv("C:\\Users\\Brian\\Desktop\\cloud_reference\\instance_series.csv", index=False)
...
if __name__ == '__main__':
    ...
    for item in vendor_list:
        ...
        series_list = series_list + temp_family_list

    to_csv(provider_list, instance_list, cpu_list, memory_list, series_list)